...
## 🆕 v13 — 2026-09-02 (3가지 추가 방어막)

### 배경 / 분석 근거

9/2 경기 결과 40.0% 적중률 (6/15). 핵심 패턴 3가지:

1. **BAL@COL**: BAL 60.7% 예측 → COL 홈 업셋. 쿠어스 필드 홈팀 이점이 추가로 미반영
2. **TB@NYM, BOS@SEA**: 불펜 70pt, 65pt 과신 → 패배. BP 점수도 극단값 캡 필요
3. **STL@LAD, MIL@CHC**: SP↔BAT 충돌 + Kalshi 마켓 없음 → 기존 룰 미발동. 오늘 15경기 전부 Kalshi 없음

---

### 변경 내용

#### 1. 쿠어스 필드 홈팀 추가 보정 (+5pt)

| 항목 | 이전 | 이후 |
|------|------|------|
| 원정팀 BAT | +6pt | +6pt (동일) |
| 홈팀(COL) BAT | +6pt | **+11pt** (홈이점 추가) |
| 로그 | `SP -8pt, 타선 +6pt` | `원정 +6pt / 홈 +11pt (홈이점 추가)` |

```python
COORS_HOME_BAT_EXTRA = 5.0  # 홈팀 추가 보정
home_bat_s = min(100.0, home_bat_s + COORS_BAT_BONUS + COORS_HOME_BAT_EXTRA)
```

---

#### 2. 불펜 극단값 캡 (BP_EXTREME_CAP = 75pt)

| 항목 | 내용 |
|------|------|
| 문제 | TB 불펜 70pt 과신 → NYM 패배 / BOS 불펜 65pt → SEA 패배 |
| 수정 | BP 점수 75pt 상한 적용 (BAT 70pt 캡과 동일 개념) |
| 로그 | `[⚠️ 불펜극단값캡] TB bp=70.0pt → 75.0pt로 제한` |

---

#### 3. SP↔BAT 충돌 + Kalshi 없음 → 자동 패스 (SP차이 25pt↑)

| 항목 | 내용 |
|------|------|
| 문제 | Kalshi 마켓 없는 날 SP↔BAT 충돌 경기를 거르지 못함 |
| 조건 | `sp_bat_conflict=True` + `kalshi_home is None` + `SP차이 ≥ 25pt` |
| 결과 | `⏭️ 패스 (SP↔BAT충돌+Kalshi없음 — 시장 미검증)` |
| 적용 케이스 | STL@LAD (SP차이 41pt) → 패스 / MIL@CHC (SP차이 38pt) → 패스 |

---

### 9/2 결과 요약 (v13 개선 전 마지막 v12 운영)
...
| 적중 | 6승 9패 |
| 적중률 | 40.0% |
| 주요 실패 | Kalshi 없음 + SP↔BAT 충돌 (STL@LAD, MIL@CHC), 불펜 과신 (TB, BOS) |
